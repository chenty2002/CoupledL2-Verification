...
template<typename T>
concept ConceptPortTopInputValids = requires {
    { std::declval<T>().io_topInputValids_0 } -> std::convertible_to<uint8_t>;
    { std::declval<T>().io_topInputValids_1 } -> std::convertible_to<uint8_t>;
};

template<typename T>
concept ConceptPortTopInputRandomAddrs = requires {
    { std::declval<T>().io_topInputRandomAddrs_0 } -> std::convertible_to<uint32_t>;
    { std::declval<T>().io_topInputRandomAddrs_1 } -> std::convertible_to<uint32_t>;
};

template<typename T>
concept ConceptPortTopInputNeedT = requires {
    { std::declval<T>().io_topInputNeedT_0 } -> std::convertible_to<uint8_t>;
    { std::declval<T>().io_topInputNeedT_1 } -> std::convertible_to<uint8_t>;
};

template<typename T>
concept ConceptPortVerifyInputAddr = requires {
    { std::declval<T>().io_0_inputAddr } -> std::convertible_to<uint32_t>;
    { std::declval<T>().io_1_inputAddr } -> std::convertible_to<uint32_t>;
};

template<typename T>
concept ConceptPortVerifyInputNeedT = requires {
    { std::declval<T>().io_0_inputNeedT } -> std::convertible_to<uint8_t>;
    { std::declval<T>().io_1_inputNeedT } -> std::convertible_to<uint8_t>;
};

struct VerifyTopInputState {
    std::array<uint32_t, 2> lanes;
};

inline static uint32_t NextVerifyWord(uint32_t& state)
{
    if (state == 0)
        state = 0x13579bdfu;
...
inline static VerifyTopInputState InitVerifyTopInputState(uint64_t seed)
{
    uint32_t base = static_cast<uint32_t>(seed);

    return VerifyTopInputState {
...
template<typename T>
inline static void V3DriveVerifyInputs(T* top, VerifyTopInputState& state, uint64_t cycle, bool resetActive)
{
    (void) state;
    (void) cycle;

    std::array<uint8_t, 2> valid = {0, 0};
    std::array<uint32_t, 2> addr = {0, 0};
    std::array<uint8_t, 2> needT = {0, 0};

    if constexpr (ConceptPortTopInputValids<T>)
    {
        top->io_topInputValids_0 = valid[0];
        top->io_topInputValids_1 = valid[1];
    }

    if constexpr (ConceptPortTopInputRandomAddrs<T>)
    {
        top->io_topInputRandomAddrs_0 = addr[0];
        top->io_topInputRandomAddrs_1 = addr[1];
    }

    if constexpr (ConceptPortTopInputNeedT<T>)
    {
        top->io_topInputNeedT_0 = valid[0] ? needT[0] : 0;
        top->io_topInputNeedT_1 = valid[1] ? needT[1] : 0;
    }

    if constexpr (ConceptPortVerifyInputAddr<T>)
    {
        top->io_0_inputAddr = resetActive ? 0 : 0;
        top->io_1_inputAddr = resetActive ? 0 : 0;
    }

    if constexpr (ConceptPortVerifyInputNeedT<T>)
    {
        top->io_0_inputNeedT = resetActive ? 0 : needT[0];
        top->io_1_inputNeedT = resetActive ? 0 : needT[1];
    }
}
...
int main(int argc, char **argv)
{
    ...
    if constexpr (ConceptPortTopInputValids<VTestTop>
            || ConceptPortTopInputRandomAddrs<VTestTop>
            || ConceptPortTopInputNeedT<VTestTop>
            || ConceptPortVerifyInputAddr<VTestTop>
            || ConceptPortVerifyInputNeedT<VTestTop>)
        std::cout << "[V3Main] \033[1;32mDriving VerifyTop external inputs from host\033[0m." << std::endl;
    else
        std::cout << "[V3Main] \033[31mNo VerifyTop external input ports detected\033[0m." << std::endl;
    ...
    std::cout << "[V3Main] LocalConfig coreCount=" << tltest->GetLocalConfig().coreCount
              << ", agentCount=" << tltest->GetAgentCount()
              << ", maximumCycle=" << tltest->GetLocalConfig().maximumCycle
              << std::endl;
...
#   endif
    ...
    fst = nullptr;
    top = new VTestTop;
    auto verifyInputState = InitVerifyTopInputState(tltest->GetLocalConfig().seed);
    ...
    V3DriveVerifyInputs(top, verifyInputState, 0, true);
    ...
    {
        V3DriveVerifyInputs(top, verifyInputState, time >> 1, false);

        if (tltest->GetAgentCount() == 0
            && tltest->GetLocalConfig().maximumCycle
            && time > tltest->GetLocalConfig().maximumCycle)
        {
            forcedVerifyFinish = true;
            break;
        }
    ...
    //
    if (forcedVerifyFinish || tltest->IsFinished())
    {
