...
class DirectoryTest(implicit p: Parameters) extends L2Module with Formal {
  ...
  println("offsetBits: ", offsetBits)
  println("bankBits: ", bankBits)
  println("setBits: ", setBits)
  ...
  val tArray = WireDefault(VecInit.fill(sets, ways)(0.U(tagBits.W)))
  stateArray.zip(metaArray.io.r).foreach { case (sArray, mArray) =>
    sArray.zip(mArray).foreach { case (s, m) =>
      s := m.state
    }
  }
  tagArray.io.r.zip(tArray).foreach { case (tag, t) =>
    tag.zip(t).foreach { case (tagA, tA) =>
      tA := tagA
    }
  }
