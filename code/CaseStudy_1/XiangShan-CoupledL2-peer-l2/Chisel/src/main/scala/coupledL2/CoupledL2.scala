...
trait HasCoupledL2Parameters {
  ...
  def parseFullAddress(x: UInt): (UInt, UInt, UInt) = {
    ...
    // val fullAddrBits = fullTagBits + setBits + offsetBits
    // val fullAddr = if (x.getWidth >= fullAddrBits) {
    //   x(fullAddrBits - 1, 0)
    // } else {
    //   Cat(0.U((fullAddrBits - x.getWidth).W), x)
    // }
    // val set = fullAddr >> offsetBits
    // val tag = set >> setBits
    // (tag(fullTagBits - 1, 0), set(setBits - 1, 0), fullAddr(offsetBits - 1, 0))
  }
  ...
  def parseAddress(x: UInt): (UInt, UInt, UInt) = {
    ...
    val tag = set >> setBits
    (tag(tagBits - 1, 0), set(setBits - 1, 0), offset(offsetBits - 1, 0))
    // val addrBits = tagBits + setBits + offsetBits + bankBits
    // val addr = if (x.getWidth >= addrBits) {
    //   x(addrBits - 1, 0)
    // } else {
    //   Cat(0.U((addrBits - x.getWidth).W), x)
    // }
    // val set = addr >> (offsetBits + bankBits)
    // val tag = set >> setBits
    // (tag(tagBits - 1, 0), set(setBits - 1, 0), addr(offsetBits - 1, 0))
  }
...
abstract class CoupledL2Base(implicit p: Parameters) extends LazyModule with HasCoupledL2Parameters {
  ...
  val managerPortParams = (m: TLSlavePortParameters) => TLSlavePortParameters.v1(
    ...
    beatBytes = (if (scala.sys.env.get("VERIFY_MODE").exists(_.trim.equalsIgnoreCase("large"))) 32 else 1),
    minLatency = 2,
