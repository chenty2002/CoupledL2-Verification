...
class TLMessageGenerator(params: MessageGeneratorParam)(implicit p: Parameters) extends LazyModule {
  ...
  class TLMessageGeneratorImp(wrapper: TLMessageGenerator) extends LazyModuleImp(wrapper) {
    ...
    // Keep Message Generator close to CoupledL2AsL1 prefetch semantics:
    // do not issue duplicate same-line acquires while one is still in-flight.
    val sameLineInflight = VecInit((0 until numSources).map { i =>
      entryActive(i) && lineBase(entryAddr(i)) === lineBase(io.in_addr)
    }).asUInt.orR
    ...
    val allocateAcquire = takeAcquire && (needAcquireBlock || needAcquirePerm) && !sameLineInflight && canAllocate
    ...
    when (allocateAcquire) {
      ...
      entryUseAcquireBlock(allocIdx) := !needAcquirePerm
