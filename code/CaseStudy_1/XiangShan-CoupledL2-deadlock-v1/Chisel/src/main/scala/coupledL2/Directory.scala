...
class DirRead(implicit p: Parameters) extends L2Bundle {
  ...
  val wayMask = UInt(cacheParams.ways.W) // all 1
  val replacerInfo = new ReplacerInfo() // chn 001/A 010/B 100/C opcode, see manual 
  // dirRead when refill
  val refill = Bool()
...
class Directory(implicit p: Parameters) extends L2Module with Formal {
  ...
  val io = IO(new Bundle() {
    ...
    // val testIoDirectoryMetaArray = Output(Vec(cacheParams.ways, UInt(stateBits.W)))
  })
  ...
  */
  ...
  // io.testIoDirectoryMetaArray := metaRead.map(_.state)

  // tag 跟 set 相同 => 同地址，然后用 resp.state, 牵出来 metaArray.io.w metaWen 有效信号，有效时更新 软件写目录
  ...
  // assume(!(io.resp.valid && io.resp.bits.hit), "Formal Verification Assume")
