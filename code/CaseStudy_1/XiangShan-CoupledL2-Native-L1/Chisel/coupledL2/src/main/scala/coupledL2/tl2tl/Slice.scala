...
class Slice()(implicit p: Parameters) extends BaseSlice[OuterBundle] {
  ...
  lazy val mainPipe = Module(new MainPipe())
  ...
  // Test modules for verification
  val directoryTest = Module(new DirectoryTest())
  val dataStorageTest = Module(new DataStorageTest())
  ...
  // Connect DataStorageTest
  dataStorageTest.io.req <> mainPipe.io.toDS.req_s3
  dataStorageTest.io.wdata := mainPipe.io.toDS.wdata_s3
  ...
  // Export signals for formal verification
  val verify_acquire_fire = WireDefault(sinkA.io.a.fire)
  val verify_grant_fire = WireDefault(grantBuf.io.d.fire)
  val verify_probe_fire = WireDefault(sinkB.io.b.fire)
  val verify_probeack_fire = WireDefault(sourceC.io.out.fire)
