...
class DataStorage(implicit p: Parameters) extends HuanCunModule {
  ...
  val nrStacks = 2
  val stackBits = log2Ceil(nrStacks) // 1
  val bankBytes = 1
  val rowBytes = nrStacks * beatBytes // 2*1 = 2
  val nrRows = sizeBytes / rowBytes // 16/2 = 8
  val nrBanks = rowBytes / bankBytes // 2/1 = 2
  val rowBits = log2Ceil(nrRows) // 3
  val stackSize = nrBanks / nrStacks // 2/2 = 1
  val sramSinglePort = true
  println(s"seesee $rowBytes $nrRows $nrBanks $rowBits $stackSize")
  ...
  val bankedData = Seq.fill(nrBanks) { //2
    Module(
      new SRAMWrapper(
        gen = UInt((8 * bankBytes).W), //8
        set = nrRows,                  //8
  ...
  def req(wen: Boolean, addr: DecoupledIO[DSAddress], data: DSData) = {
    ...
    out.bankSel := Mux(addr.valid, UIntToOH(stackIdx, 2), 0.U) // TODO: consider mask
