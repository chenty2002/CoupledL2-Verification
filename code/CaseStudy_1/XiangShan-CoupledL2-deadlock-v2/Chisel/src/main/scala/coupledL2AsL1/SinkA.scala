...
class SinkA(implicit p: Parameters) extends L2Module {
  val io = IO(new Bundle() {
    ...
    val prefetchReq = prefetchOpt.map(_ => Flipped(DecoupledIO(new PrefetchReq)))  // 11.17: 从prefectcher进入流水线, 
  ...
  def fromPrefetchReqtoTaskBundle(req: PrefetchReq): TaskBundle = {
    ...
    task.useProbeData := false.B
    task.mshrRetry := false.B
    task.fromL2pft.foreach(_ := true.B) // task.fromL2pft.foreach(_ := req.isBOP)
