...
object XSPerfRolling {
  ...
  def apply(
    ...
    if (params.enablePerf && !params.FPGAPlatform) {
      ...
      val rollingTable = ChiselDB.createTable(tableName, new RollingEntry(), basicDB=false) // val rollingTable = ChiselDB.createTable(tableName, new RollingEntry(), basicDB=true)
