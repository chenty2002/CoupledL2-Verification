...
package messageGenarator.prefetch
...
case class messageGenaratorPrefParam() extends PrefetchParameters {
...
class Prefetcher(implicit p: Parameters) extends PrefetchModule with Formal {
  ...
  val randomIO = IO(new Bundle() {
    ...
    val inputRandomAddr = Input(UInt(3.W))
  })
  ...
  prefetchOpt.get match {
    case _ =>
      ...
      val randomAddr = RegInit(1.U(8.W))
