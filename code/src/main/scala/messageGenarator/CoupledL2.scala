...
import coupledL2._
import coupledL2.prefetch.{PrefetchReceiverParams, PrefetchResp, PrefetchTrain}
import messageGenarator.prefetch._
...
//      val testPrefetchValid1 = Input(Bool())
      val prefetcherInputValid = Input(Bool())
      val prefetcherInputRandomAddr = Input(UInt(3.W))
...
//      val directory_test_metaArray = Output(Vec(banks ,Vec(cacheParams.sets, Vec(cacheParams.ways, UInt(stateBits.W)))))
//      val directory_test_tagArray = Output(Vec(banks ,Vec(cacheParams.sets, Vec(cacheParams.ways, UInt(tagBits.W)))))
    })
    ...
    prefetcher.get.randomIO.inputValid := io.prefetcherInputValid
    ...
    val slices = node.in.zip(node.out).zipWithIndex.map {
      case (((in, edgeIn), (out, edgeOut)), i) =>
        ...
        // io.testIoDirectoryMetaArray := slice.io.testIoDirectoryMetaArray

//        io.directory_test_tagArray(i) := slice.io.directory_test_tagArray
//        io.directory_test_metaArray(i) := slice.io.directory_test_metaArray
        slice
    }
