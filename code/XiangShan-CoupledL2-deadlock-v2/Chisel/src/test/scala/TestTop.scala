...
class TestTop_L2()(implicit p: Parameters) extends LazyModule {
  ...
  val xbar = TLXbar()
  val ram = LazyModule(new TLRAM(AddressSet(0, 0xffffL), beatBytes = (if (scala.sys.env.get("VERIFY_MODE").exists(_.trim.equalsIgnoreCase("large"))) 32 else 1))) // val ram = LazyModule(new TLRAM(AddressSet(0, 0xffffL), beatBytes = 32))
  ...
  ram.node :=
    TLXbar() :=*
      TLFragmenter(1, 1) :=* // TLFragmenter(32, 64) :=*
      TLCacheCork() :=*
...
object TestTop_L2 extends App {
  ...
  // (new ChiselStage).emitSystemVerilog(top.module, args)
  // (new ChiselStage).emitSystemVerilog(
  //   top.module,
  //   Array("--target-dir", "VerilogCodes/L2")
  // )
