...
lazy val chipyard = (project in file("generators/chipyard"))
  .dependsOn(testchipip, rocketchip, boom, rocketchip_blocks, rocketchip_inclusive_cache,
    ...
    compressacc, saturn, ara, firrtl2_bridge, vexiiriscv, tacit, huancun)
  .settings(libraryDependencies ++= rocketLibDeps.value)
  .settings(
    libraryDependencies ++= Seq(
...
// -----------------------------------------------
// Standalone InclusiveCache verification harness
// -----------------------------------------------
// This lightweight project depends only on the inclusive cache + minimal Rocket-Chip code
// and emits a simple L2 + 2x TL client + backing TLRAM test top for verification experiments.
lazy val inclusivecache_verify = (project in file("inclusivecache-verification"))
  .dependsOn(rocketchip_inclusive_cache)
  .settings(
    chiselSettings,
    commonSettings,
    // Keep compilation focused; users can emit FIRRTL/Verilog via Makefile
    description := "Standalone InclusiveCache L2 verification harness (TestTop)",
    Compile / scalaSource := baseDirectory.value / "src" / "test" / "scala",
    Compile / unmanagedSourceDirectories += baseDirectory.value / "src" / "main" / "scala",
    Compile / unmanagedSourceDirectories += baseDirectory.value / ".." / "generators" / "MessageGenerator" / "messageGenerator" / "src" / "main" / "scala"
  )
